...
// Run from a standalone checkout with: node scripts/validate.mjs
...
export async function validateClaudePlugin(root = pluginRoot) {
	...
	assert.equal(
		marketplace.name,
		'ecomhunt',
		'Marketplace name must remain ecomhunt.',
	);
	assert.equal(
	...
	assert(
		...
		'Marketplace must resolve the plugin from this repository root.',
	);
...
if (process.argv[1] && path.resolve(process.argv[1]) === fileURLToPath(import.meta.url)) {
	...
	console.log('PASS standalone Claude marketplace, plugin, connector configuration, and required files');
}
